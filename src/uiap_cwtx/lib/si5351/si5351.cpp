...
static uint8_t si5351_i2c_send(uint8_t addr, const uint8_t *data, uint8_t sz)
{
	return ch32v_i2c_send(addr, data, sz);
}

static uint8_t si5351_i2c_probe(uint8_t addr)
{
...
static uint8_t si5351_i2c_read(uint8_t addr, uint8_t reg, uint8_t *value)
{
	int32_t timeout = TIMEOUT_MAX;
...
{
	// Some startup code paths skip global C++ constructors; recover a sane default.
	if(i2c_bus_addr == 0)
	{
	...
	uint8_t reg_val;
	reg_val = si5351_i2c_probe(i2c_bus_addr);
...
uint8_t Si5351::si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	return si5351_i2c_send(i2c_bus_addr, buffer, bytes + 1);
...
uint8_t Si5351::si5351_write(uint8_t addr, uint8_t data)
{
	...
	return si5351_i2c_send(i2c_bus_addr, buffer, 2);
}
...
uint8_t Si5351::si5351_read(uint8_t addr)
{
	...
	if(si5351_i2c_read(i2c_bus_addr, addr, &reg_val) != 0)
	{
		return 0;
	}
